...
if (landingIntro && landingStage) {
  ...
  let landingFrame = 0;
  let landingPointerFrame = 0;
  let targetPointerX = 0;
  let targetPointerY = 0;

  const renderLandingPointer = () => {
    landingStage.style.setProperty("--landing-pointer-x", targetPointerX.toFixed(3));
    landingStage.style.setProperty("--landing-pointer-y", targetPointerY.toFixed(3));
    landingPointerFrame = 0;
  };

  const updateLandingPointer = (event) => {
    const bounds = landingStage.getBoundingClientRect();
    targetPointerX = Math.min(1, Math.max(-1, ((event.clientX - bounds.left) / bounds.width - 0.5) * 2));
    targetPointerY = Math.min(1, Math.max(-1, ((event.clientY - bounds.top) / bounds.height - 0.5) * 2));

    if (!landingPointerFrame) {
      landingPointerFrame = window.requestAnimationFrame(renderLandingPointer);
    }
  };
  ...
  const updateLandingProgress = () => {
    ...
    const rawProgress = Math.min(1, Math.max(0, window.scrollY / transitionDistance));
    const progress = rawProgress * rawProgress * (3 - 2 * rawProgress);
    const exit = Math.min(1, Math.max(0, (rawProgress - 0.84) / 0.16));
  ...
  if (landingCopy && finePointer.matches && !reducedMotion.matches) {
    landingStage.addEventListener("pointermove", updateLandingPointer, { passive: true });
    landingStage.addEventListener("pointerleave", resetLandingPointer);
  }
}
